// This file was auto-generated by the card stub generator.
// Card effects were implemented by an agent.
// If you have any questions or feedback, reach out to @C4 in the discord.

import { PokemonCard } from '../../game/store/card/pokemon-card';
import { Stage, CardType } from '../../game/store/card/card-types';
import { PlayerType, PowerType, StoreLike, State, StateUtils } from '../../game';
import { CheckProvidedEnergyEffect } from '../../game/store/effects/check-effects';
import { Effect } from '../../game/store/effects/effect';
import { WAS_ATTACK_USED, IS_ABILITY_BLOCKED, PREVENT_AND_CLEAR_SPECIAL_CONDITIONS } from '../../game/store/prefabs/prefabs';

export class GalarianRapidash extends PokemonCard {
  public stage: Stage = Stage.STAGE_1;
  public evolvesFrom: string = 'Galarian Ponyta';
  public cardType: CardType = P;
  public hp: number = 100;
  public weakness = [{ type: D }];
  public resistance = [{ type: F, value: -30 }];
  public retreat = [C];

  public powers = [{
    name: 'Pastel Veil',
    powerType: PowerType.ABILITY,
    text: 'Your Pokémon recover from all Special Conditions and can\'t be affected by any Special Conditions.'
  }];

  public attacks = [
    {
      name: 'Psychic',
      cost: [P, C],
      damage: 30,
      damageCalculation: '+' as '+',
      text: 'This attack does 30 more damage for each Energy attached to your opponent\'s Active Pokémon.'
    }
  ];

  public regulationMark: string = 'D';
  public set: string = 'SHF';
  public setNumber: string = 'SV048';
  public cardImage: string = 'assets/cardback.png';
  public name: string = 'Galarian Rapidash';
  public fullName: string = 'Galarian Rapidash SHF';

  public reduceEffect(store: StoreLike, state: State, effect: Effect): State {
    // Ability: Pastel Veil (passive - all your Pokemon immune to Special Conditions)
    // Ref: set-sword-and-shield/galarian-rapidash.ts (Pastel Veil - PREVENT_AND_CLEAR_SPECIAL_CONDITIONS)
    PREVENT_AND_CLEAR_SPECIAL_CONDITIONS(state, effect, {
      shouldApply: (target) => {
        // Find owner of the target Pokemon slot
        let owner: any = null;
        state.players.forEach(p => {
          p.forEachPokemon(PlayerType.BOTTOM_PLAYER, cardList => {
            if (cardList === target) {
              owner = p;
            }
          });
        });

        if (!owner) {
          return false;
        }

        // Check if this Galarian Rapidash is in play for the same owner
        let rapidashInPlay = false;
        owner.forEachPokemon(PlayerType.BOTTOM_PLAYER, (cardList: any, card: any) => {
          if (card === this) {
            rapidashInPlay = true;
          }
        });

        if (!rapidashInPlay) {
          return false;
        }

        return !IS_ABILITY_BLOCKED(store, state, owner, this);
      }
    });

    // Attack 1: Psychic
    // Ref: set-sword-and-shield/galarian-rapidash.ts (Psychic - CheckProvidedEnergyEffect for energy count)
    if (WAS_ATTACK_USED(effect, 0, this)) {
      const player = effect.player;
      const opponent = StateUtils.getOpponent(state, player);

      const checkProvidedEnergyEffect = new CheckProvidedEnergyEffect(opponent);
      store.reduceEffect(state, checkProvidedEnergyEffect);
      const energyCount = checkProvidedEnergyEffect.energyMap.reduce((left, p) => left + p.provides.length, 0);
      effect.damage += 30 * energyCount;
    }

    return state;
  }
}
